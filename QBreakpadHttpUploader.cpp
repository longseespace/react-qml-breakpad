...
QBreakpadHttpUploader::QBreakpadHttpUploader(QObject *parent)
    : QObject(parent), m_file(nullptr), m_uploading(false) {}
...
void QBreakpadHttpUploader::setUrl(const QUrl &url) { m_request.setUrl(url); }
...
void QBreakpadHttpUploader::uploadDump(const QString &abs_file_path,
  ...
  qDebug() << "Upload Dump" << abs_file_path;
  ...
  QHttpPart prodPart;
#if defined(SOCORRO)
  prodPart.setHeader(QNetworkRequest::ContentDispositionHeader,
                     QVariant("form-data; name=\"ProductName\"")); // Socorro
#elif defined(CALIPER)
  prodPart.setHeader(QNetworkRequest::ContentDispositionHeader,
                     QVariant("form-data; name=\"prod\"")); // Caliper
#endif
  prodPart.setBody(qApp->applicationName().toLatin1());
  // product version parameter
  QHttpPart verPart;
#if defined(SOCORRO)
  verPart.setHeader(QNetworkRequest::ContentDispositionHeader,
                    QVariant("form-data; name=\"Version\"")); // Socorro
#elif defined(CALIPER)
  verPart.setHeader(QNetworkRequest::ContentDispositionHeader,
                    QVariant("form-data; name=\"ver\"")); // Caliper
#endif
  ...
  foreach (QHttpPart custom_part, form_data) { multiPart->append(custom_part); }

  m_request.setRawHeader("User-Agent",
                         qApp->applicationName().toLatin1() + "/" +
                             qApp->applicationVersion().toLatin1());
#if defined(SOCORRO)
  m_request.setRawHeader("Host", qApp->applicationName().toLower().toLatin1() +
                                     "_reports");
...
void QBreakpadHttpUploader::onUploadFinished() {
  ...
  if (m_reply->error() != QNetworkReply::NoError) {
    qWarning("Upload error: %d - %s", m_reply->error(),
  ...
  emit finished(m_reply->error(), data);
...
bool QBreakpadHttpUploader::uploading() const { return m_uploading; }
